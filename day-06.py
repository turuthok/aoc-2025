...
arr = list(map(lambda x: ''.join(x).strip(), arr))                    # then combine into strings
arr = [list(group) for key, group in groupby(arr, key=bool) if key]   # unflatten the array into 2D, delimited by empty string
arr = [list(map(int, x)) for x in arr]                                # convert all strings to integers
